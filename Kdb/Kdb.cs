...
namespace kx
{
    ...
    public class kdb : TcpClient
    {
        ...
        */
        ...
        public kdb(string h, int p) : this(h, p, Environment.UserName) { }
 ...
 new Month(ni),new Date(ni),new DateTime(0),new KTimespan(nj),new Minute(ni),new Second(ni),new TimeSpan(nj)};
        ...
        private void u()
        {
            ...
            while (s < dst.Length)
            {
                if (i == 0) { f = 0xff & (int)b[d++]; i = 1; }
                if ((f & i) != 0) { r = aa[0xff & (int)b[d++]]; dst[s++] = dst[r++]; dst[s++] = dst[r++]; n = 0xff & (int)b[d++]; for (int m = 0; m < n; m++) dst[s + m] = dst[r + m]; }
        ...
        {
            public TimeSpan t; private KTimespan() { }
            public KTimespan(long x) { t = new TimeSpan(x == nj ? nj : x / 100); }
        ...
        {
            if (t(X) == 98) return (Flip)X; Dict d = (Dict)X; Flip a = (Flip)d.x, b = (Flip)d.y; int m = kdb.n(a.x), n = kdb.n(b.x);
...
x is DateTime[] ? 12 : x is DateTime[] ? 15 : x is KTimespan[] ? 16 : x is TimeSpan[] ? 19 : x is Flip ? 98 : x is Dict ? 99 : 0;
        ...
        static int nx(object x)
        {
            ...
            if (t < 0) return t == -11 ? 2 + ns((string)x) : 1 + nt[-t]; j = 6; n = kdb.n(x); if (t == 0 || t == 11) for (; i < n; ++i) j += t == 0 ? nx(((object[])x)[i]) : 1 + ns(((string[])x)[i]); else j += n * nt[t]; return j;
        }
        ...
        void w(bool x) { B[J++] = (byte)(x ? 1 : 0); }
        bool rb() { return 1 == b[j++]; }
        void w(byte x) { B[J++] = x; }
        byte rx() { return b[j++]; }

        void w(short h) { B[J++] = (byte)h; B[J++] = (byte)(h >> 8); }
        short rh() { int x = b[j++], y = b[j++]; return (short)(a ? x & 0xff | y << 8 : x << 8 | y & 0xff); }

        void w(int i) { w((short)i); w((short)(i >> 16)); }
        int ri() { int x = rh(), y = rh(); return a ? x & 0xffff | y << 16 : x << 16 | y & 0xffff; }
        ...
        void w(long j) { w((int)j); w((int)(j >> 32)); }
        long rj() { int x = ri(), y = ri(); return a ? x & 0xffffffffL | (long)y << 32 : (long)x << 32 | y & 0xffffffffL; }
        ...
        float re()
        {
            ...
            if (!a) { c = b[j]; b[j] = b[j + 3]; b[j + 3] = c; c = b[j + 1]; b[j + 1] = b[j + 2]; b[j + 2] = c; }
            e = BitConverter.ToSingle(b, j); j += 4; return e;
        }

        void w(double f) { w(BitConverter.DoubleToInt64Bits(f)); }
        double rf() { return BitConverter.Int64BitsToDouble(rj()); }

        void w(char c) { w((byte)c); }
        char rc() { return (char)(b[j++] & 0xff); }
        ...
        void w(TimeSpan t) { if (vt < 1) throw new KException("Timespan not valid pre kdb+2.6"); w(qn(t) ? ni : (int)(t.Ticks / 10000)); }
        TimeSpan rt() { int i = ri(); return new TimeSpan(qn(i) ? nj : 10000L * i); }
        ...
        void w(KTimespan t) { w(qn(t) ? nj : (t.t.Ticks * 100)); }
        KTimespan rn() { return new KTimespan(rj()); }

        DateTime rp() { long j = rj(), d = j < 0 ? (j + 1) / 100 - 1 : j / 100; DateTime p = new DateTime(j == nj ? 0 : o + d); return p; }

        void w(object x)
        {
            int t = kdb.t(x); w((byte)t); if (t < 0) switch (t)
                {
                    case -1: w((bool)x); return;

                    case -2: w((Guid)x); return;
            ...
            B[J++] = 0; if (t == 98) { Flip r = (Flip)x; B[J++] = 99; w(r.x); w(r.y); return; }
        ...
        object r()
        {
            ...
            if (t > 99) { if (t == 101 && b[j++] == 0) return null; throw new KException("func"); }
        ...
        public object k(string s) { return k(cs(s)); }
        char[] cs(string s) { return s.ToCharArray(); }
